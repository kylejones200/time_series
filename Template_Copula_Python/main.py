...
def create_visualizations(
    data, u, v, copula_samples, forecast1, forecast2, copula_type, config
):
    ...
    axes[1, 1].scatter(
        forecast1, forecast2, alpha=0.6, s=20, edgecolors="black", linewidths=0.5
    )
...
def main():
    ...
    data = pd.DataFrame(
        {
            config["data"]["series1_col"]: df1.loc[
                common_index, config["data"]["series1_col"]
            ],
            config["data"]["series2_col"]: df2.loc[
                common_index, config["data"]["series2_col"]
            ],
        },
        index=common_index,
    )
    ...
    copula_samples = simulate_from_copula(
        copula, config["model"]["n_samples"], config["model"]["copula_type"]
    )
