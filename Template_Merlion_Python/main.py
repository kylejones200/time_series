...
def create_forecaster(config):
    ...
    forecaster_map = {
        'Prophet': lambda: ProphetForecaster(**config['model'].get('forecaster_params', {})),
        'ARIMA': lambda: Arima(ArimaConfig(
            order=tuple(config['model'].get('arima_order', [2, 1, 2])),
            max_forecast_steps=config['model'].get('forecast_horizon', 12)
        )),
    }
    
    return forecaster_map.get(config['model']['forecaster_type'], forecaster_map['Prophet'])()


def create_anomaly_detector(config):
    """Create Merlion anomaly detector based on config."""
    detector_map = {
        'IsolationForest': lambda: IsolationForest(**config['model'].get('detector_params', {})),
        'AutoEncoder': lambda: AutoEncoder(**config['model'].get('detector_params', {})),
    }
    
    return detector_map.get(config['model']['detector_type'], detector_map['IsolationForest'])()
...
def create_anomaly_visualization(data, predictions, config):
    ...
    ax.plot(normal.index, normal.values.flatten(),
            'k-', linewidth=config['plotting']['linewidth'],
            alpha=config['plotting']['alpha'], label='Normal')
    
    ax.scatter(anomalies.index, anomalies.values.flatten(),
