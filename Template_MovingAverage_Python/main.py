...
def create_forecast(data, config):
    ...
    method_map = {
        ...
        "EMA": lambda: exponential_moving_average(
            data, config["model"].get("alpha", 0.3)
        ),
        "WMA": lambda: weighted_moving_average(
            data, window, config["model"].get("weights")
        ),
    }
...
def create_visualizations(data, ma, forecast, config):
    ...
    [
        fig.savefig(output_dir / "moving_average_forecast.png", dpi=300, bbox_inches="tight", facecolor="white")
        for _ in [None]
