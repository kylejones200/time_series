...
def create_ar1_model(data, config):
    ...
    with pm.Model() as model:
        ...
        y_obs = pm.GaussianRandomWalk("y_obs", sigma=sigma, init_dist=init_dist, shape=len(data))
...
def create_visualizations(data, trace, posterior_predictive, config):
    ...
    [posterior_samples.__setitem__(slice(None), posterior_samples[:, 0, :])
     for _ in [None] if posterior_samples.ndim > 2]
    ...
    ax.plot(data.index, posterior_mean,
            c=config['plotting']['style']['colors']['secondary'],
            linewidth=config['plotting']['style']['linewidth'],
            label='Posterior Mean')
    
    ax.fill_between(data.index, lower_bound, upper_bound,
    ...
    [save_plot(fig, output_dir / 'bayesian_forecast.png', config)
