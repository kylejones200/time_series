...
def load_ercot_data(
    data_path: Path, timestamp_col: str, load_cols: list[str]
) -> pd.DataFrame:
...
def filter_and_resample(
    df: pd.DataFrame, min_load: float, resample_freq: str
) -> pd.DataFrame:
...
def plot_series(series: pd.Series, config: dict, title: str, output_name: str) -> None:
    ...
    ax.plot(
        series.index,
        series.values,
        color=config["plotting"]["colors"][0],
...
def main() -> None:
    ...
    df = load_ercot_data(
        data_path, config["data"]["timestamp_col"], config["data"]["load_columns"]
    )

    filtered = filter_and_resample(
        df, config["processing"]["min_load"], config["processing"]["resample_frequency"]
    )
    filtered_output = (
        Path(__file__).parent / "outputs" / config["output"]["filtered_csv"]
    )
    ...
    plot_series(
        filtered.iloc[:, 0],
        config,
        config["plot_titles"]["filtered_load"],
        "filtered_load.png",
    )
    plot_series(
        diff_series,
        config,
        config["plot_titles"]["load_difference"],
        "load_difference.png",
    )
