...
1. **Massive WIP Directory** (239MB+)
   ...
   - Duplicate template notebooks (ARIMA.ipynb, Darts.ipynb, etc.)
   - Multiple cleanup/conversion scripts

2. **Duplicate Content**
   - Template notebooks in WIP that duplicate *_Python/ structure
...
3. **Remove Duplicate Template Notebooks**
   - Delete `WIP/Template_*.ipynb` files (already have *_Python/ versions)
...
rm WIP/Template_*.ipynb  # Note: These reference old Template_* naming
```
...
```
time_series/
├── templates/              # All *_Python/ moved here
│   ├── forecasting/
│   ├── anomaly_detection/
│   └── ...
