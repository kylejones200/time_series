...
def main():
    ...
    if config["model"]["method"] == "bayesian":
        trace, model = detect_change_point_bayesian(
            df[config["data"]["value_col"]], config
        )
        ...
        print(
            f"95% Credible Interval for tau: [{int(np.percentile(tau_samples, 2.5))}, {int(np.percentile(tau_samples, 97.5))}]"
        )
        ...
        axes[1, 0].hist(
            lambda_1_samples, bins=50, edgecolor="black", alpha=0.7, label="λ₁"
        )
        axes[1, 0].hist(
            lambda_2_samples, bins=50, edgecolor="black", alpha=0.7, label="λ₂"
        )
        ...
        tau, p_value = detect_change_point_frequentist(
            df[config["data"]["value_col"]], config
        )
        ...
        fig, ax = plt.subplots(figsize=
            config["plotting"]["figure_size"], config["plotting"]["dpi"]
        )
