...
def load_series(config: dict) -> pd.Series:
    ...
    df[config['data']['date_col']] = pd.to_datetime(df[config['data']['date_col']], errors='coerce')
...
def plot_differences(series_list: list[pd.Series], config: dict, adf_results: list[dict]) -> None:
    figure_cfg = config['plotting']
    fig, axes = plt.subplots(len(series_list), 1,
                            figsize=figure_cfg['figure_size'],
                            sharex=True)
    if len(series_list) == 1:
        axes = [axes]

    for idx, (series, ax) in enumerate(zip(series_list, axes)):
        apply_plot_style(ax, {'plotting': figure_cfg})
