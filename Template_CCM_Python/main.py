...
def create_visualizations(series1, series2, series1_name, series2_name, results, config):
    ...
    axes[0, 0].plot(series1_aligned, 'k-', linewidth=config['plotting']['linewidth'],
                    alpha=config['plotting']['alpha'], label=series1_name)
    axes[0, 0].plot(predictions_2_to_1[:min_len_1], 'r--', linewidth=config['plotting']['linewidth'],
                    alpha=config['plotting']['alpha'], label=f'Predicted from {series2_name}')
    axes[0, 0].set_title(f'CCM: {series2_name} → {series1_name}\n(Correlation: {results["series2_to_series1"]:.3f})')
    axes[0, 0].set_xlabel('Time')
    axes[0, 0].set_ylabel('Value')
    apply_legend(axes[0, 0], config['plotting']['legend'])
    
    axes[0, 1].plot(series2_aligned, 'k-', linewidth=config['plotting']['linewidth'],
                    alpha=config['plotting']['alpha'], label=series2_name)
    axes[0, 1].plot(predictions_1_to_2[:min_len_2], 'r--', linewidth=config['plotting']['linewidth'],
                    alpha=config['plotting']['alpha'], label=f'Predicted from {series1_name}')
    axes[0, 1].set_title(f'CCM: {series1_name} → {series2_name}\n(Correlation: {results["series1_to_series2"]:.3f})')
    axes[0, 1].set_xlabel('Time')
    axes[0, 1].set_ylabel('Value')
    apply_legend(axes[0, 1], config['plotting']['legend'])
    
    axes[1, 0].scatter(series1_aligned, predictions_2_to_1[:min_len_1],
                       alpha=0.6, s=20, edgecolors='black', linewidths=0.5)
    axes[1, 0].plot([series1_aligned.min(), series1_aligned.max()],
    ...
    axes[1, 1].scatter(series2_aligned, predictions_1_to_2[:min_len_2],
                       alpha=0.6, s=20, edgecolors='black', linewidths=0.5)
    axes[1, 1].plot([series2_aligned.min(), series2_aligned.max()],
