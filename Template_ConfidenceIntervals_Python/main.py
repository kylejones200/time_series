...
def main():
    ...
    if config["model"]["method"] == "bootstrap":
        mean, lower, upper = bootstrap_ci(
            train_data,
            config["model"]["n_bootstrap"],
            alpha,
            forecast_steps,
            arima_order,
        )
    else:
        mean, lower, upper = parametric_ci(
            train_data, forecast_steps, alpha, arima_order
        )
    ...
    ax.plot(
        train_df.index,
        train_df[config["data"]["value_col"]].values,
        "k-",
    ...
    ax.fill_between(
        forecast_dates, lower, upper, color="r", alpha=0.2, label=f"{ci_level:.0f}% CI"
    )
