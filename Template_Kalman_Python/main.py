...
def load_data(config):
    ...
    data_path = (
        Path(__file__).parent.parent / "data" / Path(config["data"]["input_file"]).name
    )
    ...
    [df.__setitem__(date_col, pd.to_datetime(df[date_col])) for _ in [None] if date_col]
...
def create_visualizations(measurements, estimates, config):
    ...
    [
        fig.savefig(output_dir / "kalman_filter.png", dpi=300, bbox_inches="tight", facecolor="white")
        for _ in [None]
