...
def create_visualizations(series1, series2, te_df, static_te, config):
    ...
    axes[1, 0].bar(
        [0, 1],
        [static_te["te_1_to_2"], static_te["te_2_to_1"]],
        edgecolor="black",
        alpha=0.7,
        width=0.6,
    )
    ...
    axes[1, 0].set_xticklabels(
        [te_df.columns[0], te_df.columns[1]], rotation=45, ha="right"
    )
...
def main():
    ...
    static_te_1_to_2 = compute_transfer_entropy(
        series1, series2, config["model"]["k"], config["model"]["bins"]
    )
    static_te_2_to_1 = compute_transfer_entropy(
        series2, series1, config["model"]["k"], config["model"]["bins"]
    )

    print("\nTransfer Entropy Results:")
    print("=" * 70)
    print(
        f"{config['data']['series1_name']} → {config['data']['series2_name']}: {static_te_1_to_2:.4f}"
    )
    print(
        f"{config['data']['series2_name']} → {config['data']['series1_name']}: {static_te_2_to_1:.4f}"
    )
